...
fn render_rust_method(operation: &Operation) -> String {
    ...
    format!(
        "    pub async fn {}(&self{} ) -> Result<String, SdkError> {{\n        {}\n        self.request(Method::{} , &path).await\n    }}\n\n",
...
pub(crate) fn render_readme(spec: &ApiSpec) -> String {
    format!(
        "# {} SDK\n\nGenerated by GodSDK from OpenAPI {}.\n\nThe Rust SDK is async-first and uses Tokio. Run the generated integration tests with:\n\n```sh\ncargo test --manifest-path sdk/rust/Cargo.toml --locked\n```\n\nUse it from an async Tokio application:\n\n```rust\nlet client = Client::builder(\"https://api.example.com\").bearer_token(token).build()?;\nlet response = client.get_pet(\"pet-1\").await?;\n```\n",
